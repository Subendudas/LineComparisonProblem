...
public class LineComp_OOps {
	...
	float[] input2 = inputValues2(scan);
...
private static float[] inputValues(Scanner input) {
	System.out.print("Enter the value for Line-A (x1,y1) and (x2,y2): ");
...
private static float[] inputValues2(Scanner input) {
	System.out.print("Enter the value for Line-B (a1,b1) and (a2,b2): ");
	float a1 = input.nextFloat();
	float b1 = input.nextFloat();
	float a2 = input.nextFloat();
	float b2 = input.nextFloat();

	return new float[] { a1, b1, a2, b2 };
}
